...
const ProductDetails = () => {
  ...
  const product = products.find((item) => item._id === id); // ✅ Works if id is string

  useEffect(() => {
    if (products.length > 0 && product) {
      const related = products.filter((item) => item.category === product.category && item._id !== id);
  ...
  useEffect(() => {
    if (product?.image?.length) {
      setThumbnail(product.image[0]); // ✅ Using image[0]
    }
  }, [product]);
  ...
  return (
    <div className="mt-12">
      <p className="text-sm text-gray-600 mb-4">
        <Link to="/">Home</Link> /
        <Link to="/products" className="ml-1">Products</Link> /
        <Link to={`/products/${product.category.toLowerCase()}`} className="ml-1 capitalize">
          {product.category}
        </Link> /
      ...
      <div className='flex flex-col items-center mt-20'>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-6  mt-6 w-full'>
          {relatedProducts.filter(()=>product.inStock).map
        ((product, index)=>(
